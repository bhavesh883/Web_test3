let rem = document.querySelector(".remove-btn");
let modalCont = document.querySelector(".modal-cont");
let hum = document.querySelector(".textarea-cont");
let ape = document.querySelector(".add-btn");
let kim = document.querySelectorAll(".priority-color");
...
kim.forEach((ColorElem, idx) => {
    ColorElem.addEventListener("click", (e) => {
        kim.forEach((priorityColorElem, idx) => {
...
rem.addEventListener("click", (e) => {
    ...
    if(removeFlag)
    {
        rem.classList.add("wei");
    }
    else
    {
        rem.classList.remove("wei");
    }
...
modalCont.addEventListener("keydown", (e) => {
    let key = e.key;
    if (key === "Enter") {
        createTicket(modalPriorityColor, hum.value);
        addFlag = false;
        setModalToDefault();
    }
})
...
function setModalToDefault(){
    ...
    kim[kim.length - 1].classList.add("border");
